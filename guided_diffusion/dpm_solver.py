...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        if model_type == "noise":
            ...
            pred = (x - alpha_t * model_output) / sigma_t
        elif model_type == "v":
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            pred = alpha_t * model_output + sigma_t * x
        elif model_type == "score":
            sigma_t = noise_schedule.marginal_std(t_continuous)
            pred = -sigma_t * model_output
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            pred, cal = noise_pred_fn(x, t_continuous)
            return (pred, cal) if cal is not None else pred
        elif guidance_type == "classifier":
            assert classifier_fn is not None
            t_input = get_model_input_time(t_continuous)
            cond_grad = cond_grad_fn(x, t_input)
            sigma_t = noise_schedule.marginal_std(t_continuous)
            noise, cal = noise_pred_fn(x, t_continuous)
            pred = noise - guidance_scale * sigma_t * cond_grad
            return (pred, cal) if cal is not None else pred
        elif guidance_type == "classifier-free":
            if guidance_scale == 1. or unconditional_condition is None:
                pred, cal = noise_pred_fn(x, t_continuous, cond=condition)
                return (pred, cal) if cal is not None else pred
            else:
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t_continuous] * 2)
                c_in = torch.cat([unconditional_condition, condition])
                noise_tensor, cal_tensor = noise_pred_fn(x_in, t_in, cond=c_in)
                noise_uncond, noise = noise_tensor.chunk(2)
                pred = noise_uncond + guidance_scale * (noise - noise_uncond)
                if cal_tensor is not None:
                    _, cal = cal_tensor.chunk(2)
                    return pred, cal
                else:
                    return pred
