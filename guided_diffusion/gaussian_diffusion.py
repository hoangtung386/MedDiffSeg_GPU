...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # The model may output both mean and variance predictions,
            # but we use a fixed variance.
    ...
    def p_sample_loop_known(
        ...
        if self.dpm_solver:
            ...
            img_condition = img[:, :-1, ...].to(device)
            def conditioned_model_fn(x, t, **kwargs):
                # x is the noisy mask from DPM-Solver, t is the timestep
                full_input = torch.cat([img_condition, x], dim=1)
                return model(full_input, t, **kwargs)

            model_fn = model_wrapper(
                conditioned_model_fn,
                noise_schedule,
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                # The model may output both mean and variance predictions,
                # but we use a fixed variance, so we ignore the variance output.
                if model_output.shape[1] == 2:
                    model_output, _ = th.split(model_output, 1, dim=1)
